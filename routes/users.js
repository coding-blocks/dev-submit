...
router.post('/register', (req, res) => {
    ...
    if (errors) {
        ...
        if (User.role == "Student") {
            db.addUser(data => {
                db.addStudent(User.name, User.roll, User.email, data.dataValues.id, (responseData) => {
                    db.addLocalUser(User.username, User.password, data.dataValues.id, () => {
                        console.log(responseData.dataValues);
                        req.flash("success_msg", "You are registered as Student and can now log in");
            ...
            db.addUser(data => {
                db.addTeacher(User.name, User.email, data.dataValues.id, (responseData) => {
                    db.addLocalUser(User.username, User.password, data.dataValues.id, () => {
                        console.log(responseData.dataValues);
                        req.flash("success_msg", "You are registered as Teacher and can now log in");
                        res.redirect('/users/login');
                    });
                });
            });
